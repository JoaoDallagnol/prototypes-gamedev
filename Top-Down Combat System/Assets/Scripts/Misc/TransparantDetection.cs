...
public class TransparantDetection : MonoBehaviour {
    [Range(0,1)]
    [SerializeField] private float transparenceAmount = 0.8f;
    ...
    private void OnTriggerEnter2D(Collider2D other) {
        if (other.gameObject.GetComponent<PlayerController>()) {
            if (spriteRenderer){
                StartCoroutine(FadeRoutine(spriteRenderer, fadeTime, spriteRenderer.color.a, transparenceAmount));
            } else if (tilemap) {
                StartCoroutine(FadeRoutine(tilemap, fadeTime, tilemap.color.a, transparenceAmount));
            }
        }
    }
